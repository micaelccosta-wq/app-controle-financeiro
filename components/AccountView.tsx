...
const AccountView: React.FC<AccountViewProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const accountData: Account = {
      ...
      type,
      type,
      initialBalance: (type === AccountType.BANK || type === AccountType.INVESTMENT) ? (parseFloat(initialBalance) || 0) : 0,
  ...
  const calculateCurrentBalance = (account: Account) => {
    // Only applies for BANK and INVESTMENT accounts
    if (account.type === AccountType.CREDIT_CARD) return 0;
  ...
  const totalAllAccounts = accounts
    .filter(a => a.type === AccountType.BANK || a.type === AccountType.INVESTMENT)
    .reduce((acc, account) => acc + calculateCurrentBalance(account), 0);

  // Set default account for reconciliation on load
  useEffect(() => {
    if (!selectedAccountId && accounts.length > 0) {
      const defaultBank = accounts.find(a => a.type === AccountType.BANK && a.isDefault);
      if (defaultBank) {
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="isDefault" className="text-sm text-slate-700 select-none cursor-pointer flex items-center gap-1">
                Definir como padrão para {type === AccountType.CREDIT_CARD ? 'Cartões' : type === AccountType.INVESTMENT ? 'Investimentos' : 'Contas Correntes'}
                <Star size={12} className={isDefault ? "fill-amber-400 text-amber-400" : "text-slate-400"} />
              </label>
            </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {accounts.filter(a => a.type === AccountType.CREDIT_CARD).length === 0 ? (
              <div className="col-span-full p-6 text-center text-slate-400 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
        ...
        <div className="space-y-4">
          <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2">
            ...
            Contas Correntes / Carteira
          </h3>
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div className="divide-y divide-slate-100 dark:divide-slate-700">
              {accounts.filter(a => a.type === AccountType.BANK).length === 0 ? (
                <div className="p-6 text-center text-slate-400">Nenhuma conta cadastrada.</div>
              ) : (
                accounts.filter(a => a.type === AccountType.BANK).map(acc => {
                  const current = calculateCurrentBalance(acc);
                  return (
                    <div key={acc.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <p className="font-medium text-slate-800 dark:text-slate-100">{acc.name}</p>
                            {acc.isDefault && <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded border border-amber-200 flex items-center gap-1"><Star size={8} className="fill-amber-700" /> Padrão</span>}
                          </div>
                          <p className="text-xs text-slate-500 dark:text-slate-400">Inicial: {acc.initialBalance.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-6">
                        <div className="text-right">
                          <p className="text-xs text-slate-400 uppercase font-bold tracking-wider">Saldo Atual</p>
                          <p className={`font-bold ${current >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                            {current.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                          </p>
                        </div>
                        <div className="flex gap-1">
                          <button onClick={() => handleEdit(acc)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Edit2 size={16} /></button>
                          <button onClick={() => onDeleteAccount(acc.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"><Trash2 size={16} /></button>
                        </div>
                      </div>
                    </div>
                  );
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div className="divide-y divide-slate-100 dark:divide-slate-700">
              {accounts.filter(a => a.type === AccountType.INVESTMENT).length === 0 ? (
                <div className="p-6 text-center text-slate-400">Nenhum investimento cadastrado.</div>
              ) : (
                accounts.filter(a => a.type === AccountType.INVESTMENT).map(acc => {
                  const current = calculateCurrentBalance(acc);
                  return (
                    <div key={acc.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                      <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-6">
                        <div className="text-right">
                          <p className="text-xs text-slate-400 uppercase font-bold tracking-wider">Saldo Atual</p>
                          <p className={`font-bold ${current >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-rose-600 dark:text-rose-400'}`}>
                            {current.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                          </p>
                        </div>
                        <div className="flex gap-1">
                ...
                {actualBalanceInput && (
                  <div className="w-full sm:w-auto">
                    <div className="bg-slate-100 rounded-lg px-4 py-2 text-right min-w-[140px]">
                      ...
                      <p className={`font-bold ${(parseFloat(actualBalanceInput) - calculatedSystemBalance) === 0 ? 'text-emerald-500' : 'text-amber-500'
                        }`}>
