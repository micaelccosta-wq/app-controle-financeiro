...
const WealthView: React.FC<WealthViewProps> = ({
  ...
  const averageMonthlyPassiveIncome = useMemo(() => {
     // Filter income transactions classified as 'Rendimentos'
     ...
     const total = incomeTransactions.reduce((acc, t) => acc + t.amount, 0);
  ...
  const averageFixedExpenses = useMemo(() => {
     ...
     const fixedExpenses = transactions.filter(t => 
        ...
        t.isApplied &&
        fixedCategoryNames.includes(t.category)
     );

     const total = fixedExpenses.reduce((acc, t) => acc + t.amount, 0);
     return total / monthSpan;
     ...
     const getSum = (year: number, catName?: string) => {
        return transactions
           .filter(t => {
              ...
              if (catName && t.category !== catName) return false;
              // If no catName provided, verify if transaction category impacts budget
              if (!catName) {
                 const cat = categories.find(c => c.name === t.category);
                 return cat && cat.impactsBudget;
              }
              return true;
           })
           .reduce((acc, t) => acc + t.amount, 0);
                  ...
                  {inflationData.list.map(item => (
                     <tr key={item.category} className="hover:bg-slate-50">
                        ...
                        <td className={`px-4 py-3 text-right font-bold ${
                           (item.inflation1yr || 0) > 0 ? 'text-rose-600' : 'text-emerald-600'
                        }`}>
